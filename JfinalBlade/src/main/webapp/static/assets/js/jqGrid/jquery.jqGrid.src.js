...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox headcbox' type='checkbox'/>");
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			var icb = ts.p.colModel[i].name == 'cb' ? "class='jqgh-cb'":"";
			thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+icb+tdc+">"+ts.p.colNames[i];
